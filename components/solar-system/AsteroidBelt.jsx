...
export default function AsteroidBelt({ count = 2000, innerRadius = 50, outerRadius = 65, timeMultiplier = 1 }) {
  ...
  if (!beltData.current) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Slightly larger than original, but not massive, so they look like small chunks of rock
      const scale = 0.08 + Math.pow(Math.random(), 3) * 0.4
      ...
      // Asteroid colors: Much brighter and dustier so they don't look like dark insects against space
      const type = Math.random()
      if (type < 0.5) {
        colorObj.setHSL(0, 0, 0.45 + Math.random() * 0.15) // C-type: lighter stony grey
      } else if (type < 0.8) {
        colorObj.setHSL(0.08, 0.2 + Math.random() * 0.15, 0.55 + Math.random() * 0.15) // S-type: dusty tan
      } else {
        colorObj.setHSL(0, 0, 0.65 + Math.random() * 0.15) // M-type: bright metallic grey
      }
